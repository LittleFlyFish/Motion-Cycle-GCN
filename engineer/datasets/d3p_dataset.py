...
class Pose3dPW(Dataset):
    ...
    def __init__(self, path_to_data, input_n=20, output_n=10, dct_n=15, split=0):
        ...
        seq_len_n = their_input_n + output_n
        ...
        for f in files:
            with open(self.data_path + f, 'rb') as f:
                ...
                joint_pos = data['poses']
                for i in range(len(joint_pos)):
                    seqs = joint_pos[i]

                    n_frames = seqs.shape[0]
                    fs = np.arange(0, n_frames - seq_len_n + 1)
                    fs_sel = fs
                    for j in np.arange(seq_len_n - 1):
