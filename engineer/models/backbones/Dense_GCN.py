...
class Dense_GCN(nn.Module):
    ...
    def __init__(self, input_feature, hidden_feature, p_dropout, num_stage=1, node_n=48):
        ...
        self.gcbs = []
        for i in range(num_stage):
            self.gcbs.append(GC_Block_NoRes(i*middle_feature+hidden_feature, middle_feature, p_dropout=p_dropout, node_n=node_n))

        self.gcbs = nn.ModuleList(self.gcbs)

        self.gc7 = GraphConvolution(middle_feature*num_stage+hidden_feature, input_feature, node_n=node_n)
