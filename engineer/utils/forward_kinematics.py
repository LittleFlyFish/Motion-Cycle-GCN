...
def fkl_torch(angles, parent, offset, rotInd, expmapInd, cuda ='cuda:0'):
    """
    ...
    j_n = offset.shape[0]
    p3d = Variable(torch.from_numpy(offset)).float().cuda(cuda).unsqueeze(0).repeat(n, 1, 1)
    angles = angles[:, 3:].contiguous().view(-1, 3)
    R = data_utils.expmap2rotmat_torch(angles, cuda=cuda).view(n, j_n, 3, 3)
