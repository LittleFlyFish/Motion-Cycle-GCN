...
def rotmat2euler_torch(R, cuda='cuda:0'):
    """
    Converts a rotation matrix to euler angles
    ...
    if len(idx_spec2) > 0:
        R_spec2 = R[idx_spec2, :, :]
        eul_spec2 = Variable(torch.zeros(len(idx_spec2), 3).float()).cuda(cuda)
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = Variable(torch.zeros(len(idx_remain), 3).float()).cuda(cuda)
...
def expmap2rotmat_torch(r, cuda = 'cuda:3'):
    """
    ...
    n = r1.data.shape[0]
    R = Variable(torch.eye(3, 3).repeat(n, 1, 1)).float().cuda(cuda) + torch.mul(
        torch.sin(theta).unsqueeze(1).repeat(1, 9).view(-1, 3, 3), r1).cuda(cuda)  + torch.mul(
        (1 - torch.cos(theta).unsqueeze(1).repeat(1, 9).view(-1, 3, 3)), torch.matmul(r1, r1)).cuda(cuda)
    return R
    ...
    for subj in subjects:
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                ...
                the_seq2[:, 0:6] = 0
                p3d2 = expmap2xyz_torch(the_seq2, cuda)
                the_sequence2 = p3d2.view(num_frames2, -1).cpu().data.numpy()
