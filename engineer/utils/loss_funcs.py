...
def euler_error(outputs, all_seq, input_n, dim_used, dct_n, cuda='cuda:0'):
    """
...
def mpjpe_error(outputs, all_seq, input_n, dim_used, dct_n, cuda='cuda:0'):
    """
    ...
    targ_p3d = data_utils.expmap2xyz_torch(targ_expmap, cuda=cuda).view(-1, 3)

    pred_p3d = data_utils.expmap2xyz_torch(pred_expmap, cuda=cuda).view(-1, 3)
...
def mpjpe_error_p3d_seq2seq(outputs, all_seq, dct_n, dim_used):
    """
...
def mpjpe_error_p3d(outputs, all_seq, dct_n, dim_used, cuda="cuda:0"):
    ...
    _, idct_m = data_utils.get_dct_matrix(seq_len)
    idct_m = Variable(torch.from_numpy(idct_m)).float().cuda(torch.device(cuda))
...
def mpjpe_error_p3d_offset(outputs, all_seq, dct_n, dim_used):
    """
    :param outputs:n*66*dct_n
    :param all_seq:
    :param dct_n:
