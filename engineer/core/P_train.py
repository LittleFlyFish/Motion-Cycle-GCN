...
def train(train_loader, model, optimizer, lr_now=None, max_norm=True, is_cuda=False, dim_used=[], dct_n=15):
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        P_inputs = targets[:, leftdim, :] # inputs:[16, 66, 15], all_seq:[16, 20, 96]
        outputs = model(P_inputs)

        P_outputs = targets
        P_outputs[:, rightdim, :] = outputs



        # calculate loss and backward
        loss = loss_funcs.mpjpe_error_p3d(P_outputs, all_seq, dct_n, dim_used)
        optimizer.zero_grad()
        loss.backward()
        if max_norm:
...
def test(train_loader, model, input_n=20, output_n=50, is_cuda=False, dim_used=[], dct_n=15):
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        P_outputs_3d = torch.matmul(idct_m[:, 0:dct_n], P_outputs_t).transpose(0, 1).contiguous().view(-1, dim_used_len,
                                                                                                   seq_len).transpose(1,
                                                                                                                      2)
...
def val(train_loader, model, is_cuda=False, dim_used=[], dct_n=15):
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        m_err = loss_funcs.mpjpe_error_p3d(P_outputs, all_seq, dct_n, dim_used)
