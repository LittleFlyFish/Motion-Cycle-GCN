...
def train_model(model,datasets,cfg,distributed,optimizer):
    ...
    for epoch in range(start_epoch, cfg.total_epochs):
        ...
        lr_now, t_l, train_num = train(train_loader, model, optimizer, lr_now=lr_now, max_norm=cfg.max_norm, is_cuda=is_cuda,
                            ...
                            input_n=cfg.data.train.input_n,output_n=cfg.data.train.output_n, num = train_num)
...
def train(train_loader, model, optimizer, lr_now=None, max_norm=True, is_cuda=False,
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        all_seq_dim_used = all_seq[:, :, dim_used]
        padding_seq = all_seq_dim_used[:, padding_idx, :]
        # predict loss
        ...
        num += 1
        plotter.plot('loss', 'train', 'Gv=G, Lamda=1, padding=True', num, loss.item())
...
def test(train_loader, model, input_n=20, output_n=50, is_cuda=False, dim_used=[], dct_n=15):
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        plotter.plot('loss', 'test', 'Gv=G, Lamda=1, padding=True', i, test_loss.item())
...
def val(train_loader, model, is_cuda=False, dim_used=[], dct_n=15):
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        plotter.plot('loss', 'val', 'Gv=G, Lamda=1, padding=True', i, m_err.item())
