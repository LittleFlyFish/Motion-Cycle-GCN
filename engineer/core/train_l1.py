...
def train_model(model, datasets, cfg, distributed, optimizer):
    ...
    script_name = script_name + '_3D_in{:d}_out{:d}_dct_n_{:d}_L1'.format(cfg.data.train.input_n, cfg.data.train.output_n,
    ...
    for epoch in range(start_epoch, cfg.total_epochs):
        ...
        lr_now, t_l, train_num, train_loss_plot = train(train_loader, model, optimizer, lr_now=lr_now, max_norm=cfg.max_norm,
                                       is_cuda=is_cuda,dim_used=train_dataset.dim_used, dct_n=cfg.data.train.dct_used, 
                                       num=train_num, loss_list = train_loss_plot)
        ret_log = np.append(ret_log, [lr_now, t_l])
        head = np.append(head, ['lr', 't_l'])

        # val evaluation
        v_3d, val_loss_plot = val(val_loader, model, is_cuda=is_cuda, dim_used=train_dataset.dim_used, dct_n=cfg.data.val.dct_used, loss_list = val_loss_plot)
        ...
        for act in acts:
            test_l, test_3d, test_loss_plot = test(test_loaders[act], model, input_n=cfg.data.test.input_n,
                                   output_n=cfg.data.test.output_n, is_cuda=is_cuda,
                                   dim_used=train_dataset.dim_used, dct_n=cfg.data.test.dct_used, loss_list = test_loss_plot)
...
def train(train_loader, model, optimizer, lr_now=None, max_norm=True, is_cuda=False, dim_used=[], dct_n=15, num=1, loss_list=[]):
    t_l = utils.AccumLoss()
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        _, loss = loss_funcs.mpjpe_error_p3d_l1(outputs, all_seq, dct_n, dim_used)
        num += 1
...
#
#
def test(train_loader, model, input_n=20, output_n=50, is_cuda=False, dim_used=[], dct_n=15, loss_list=[]):
    N = 0
    t_l = 0
    if output_n == 25:
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        bar.next()
    bar.finish()
    return t_l / N, t_3d / N, loss_list


#
#
def val(train_loader, model, is_cuda=False, dim_used=[], dct_n=15, loss_list=[]):
